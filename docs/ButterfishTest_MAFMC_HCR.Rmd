...
Now try using the original model as is and extending to projections. changed some dims here (waa from 5 in the example to 2)
...
for (iy in (n_hist_years + 1L):n_proj_years) {
  user_maturity[, iy, ] <- OM$input$data$mature[n_hist_years, , drop = FALSE] #[, n_hist_years, , drop = FALSE]
}

user_waa <- list()
user_waa$waa <- array(NA_real_, dim = c(2, n_proj_years, n_ages))
...
index_info <- list(
  ...
  index_Neff      = c(10.967742,  4.838710,  1.677419,  4.967742,  1.064516, 0.000000), #rep(25, n_indices),
  fracyr_indices  = c(0.75, 0.75, 0.75, 0.25, 0.3333333, 0.75),
  q               = c(0.197517, 0.5090928, 0.2124225, 0.4953055, 0.0892906, 4.3494e-05), #rep(0.2, n_indices), OM$rep$q
  use_indices     = rep(1, n_indices),
  use_index_paa   = rep(1, n_indices),
  units_indices   = rep(2, n_indices),
  units_index_paa = rep(2, n_indices)
...
sel3 <- list(
  ...
  model = rep("age-specific", 8), # 2 selectivity blocks for fishery, how to deal with here?
  re    = rep("none", 8),
  initial_pars = list(
    c(0.01665252, 0.1994395, 0.6751271,    1, 0.6039584),
    c(0.004589983, 0.09634877, 0.4255508,    1, 0.5450962),
    c(1, 0.5879402, 0.6179548, 0.1687678, 0.05946685),
    c(1, 0.850923, 0.7722096, 0.391161, 0.056252),
    c(1,    1, 0.4837531, 0.290685, 0.07789516),
    c(0,    1, 0.3997233, 0.3843249, 0.2045156),
    c(0.03848512,    1,    1, 0.7092893, 0.4966039),
    c(1,    0,    0,    0,   0)
  ),
  fix_pars = list(4, 4, 1, 1, 1:2, 1, 2:3, 1) # fix at least one at 1 for age specific
)
...
M <- list(
  model = "constant",
  initial_means = array(
    c(0.2452964, 0.2452964, 0.2452964, 0.2452964, 0.2452964),
    dim = c(n_stocks, n_regions, n_ages)
  )
)
...
om_input_P$par$catch_paa_pars <- OM$parList$catch_paa_pars <- matrix(0, basic_info$n_fleets, 3) #these were not in the OM
om_input_P$par$index_paa_pars <- OM$parList$index_paa_pars <- matrix(0, basic_info$n_indices, 3) #these were not in the OM
om_input_P$par$sel_repars     <- OM$parList$sel_repars

for (i in 1:n_regions) {
  om_input_P$par$log_N1[i, i, ] <- log(OM$rep$NAA[i, 1])  #log(OM$rep$N1)[i, i, ]
}
